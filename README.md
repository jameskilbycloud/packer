# Packer — Ubuntu vSphere Templates

Automated golden-image pipeline that builds Ubuntu Server and Desktop VM templates directly in vSphere using [HashiCorp Packer](https://www.packer.io/). Three Ubuntu LTS versions are supported (22.04, 24.04, 26.04), each producing a server and a desktop variant — six templates in total.
...
2. **Packer build** — the `vsphere-iso` builder boots a VM from the ISO, attaches a cloud-init seed disc (labelled `cidata`) containing the autoinstall configuration, and waits for Ubuntu to install itself unattended.
3. **Provisioning** — shell provisioners run `setup.sh` (apt upgrade, SSH hardening, disk zeroing) and `vmtools.sh` (open-vm-tools verification).
...
│   └── desktop-user-data.pkrtpl   # Cloud-init autoinstall config — desktop
│
├── scripts/
│   ├── upload-isos.sh              # Download ISOs and upload to Content Library
│   ├── setup.sh                    # Post-install: apt upgrade, SSH hardening
│   └── vmtools.sh                  # Verifies / installs open-vm-tools
│
...
make build-all       # All six images (sequential)
```
