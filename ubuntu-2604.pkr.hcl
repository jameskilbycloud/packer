...
source "vsphere-iso" "ubuntu-2604-server" {
  ...
  dynamic "content_library_destination" {
    ...
    content {
      ...
      description = "Ubuntu 26.04 LTS Server — Packer ${local.build_timestamp} | git: ${var.git_commit}"
      ovf         = true
    }
  }
}
...
source "vsphere-iso" "ubuntu-2604-desktop" {
  ...
  # Boot — see server source comment above for the ipv6.disable + overlay
  # cmdline rationale (the desktop install path is identical until
  # desktop.sh runs over SSH, so the same kernel-level mitigations apply
  ...
  dynamic "content_library_destination" {
    ...
    content {
      ...
      description = "Ubuntu 26.04 LTS Desktop — Packer ${local.build_timestamp} | git: ${var.git_commit}"
      ovf         = true
    }
  }
}
